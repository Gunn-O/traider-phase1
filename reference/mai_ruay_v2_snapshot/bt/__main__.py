...
def main(argv=None) -> int:
    ...
    sw = sub.add_parser("sweep", help="รัน sweep หลาย variant จาก spec (grid/axis) → runs/sweeps/<name>/")
    sw.add_argument("--spec", required=True, help="sweep spec YAML (name/base/mode/data/vary/...)")
    ...
    if args.cmd == "viewer":
        return _viewer(args)
    if args.cmd == "sweep":
        from bt import sweep   # lazy: bt run ไม่ต้องมี deps ของ sweep
        return sweep.run_sweep(args.spec)
    return 1
