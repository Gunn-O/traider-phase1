...
# default schema (mai_ruay) — ใช้เมื่อ plan_meta ว่าง (ไม่มี row ให้ดึง key) เพื่อคง header เดิม
# strategy อื่น (เช่น mountain) ใส่ key คนละชุดได้ → write_plan_meta_csv เขียนคอลัมน์ตาม key จริง (dynamic)
PLAN_META_FIELDS = [
...
def _plan_meta_fields(plan_meta: list[dict]) -> list[str]:
    """คอลัมน์ = union ของ key ทุก row (ลำดับ first-seen) · บังคับ plan_id นำหน้า, reasons ปิดท้าย
    → mai_ruay ได้ header เดิมเป๊ะ (byte-identical) · strategy อื่นได้คอลัมน์ของตัวเองไหลผ่าน"""
    if not plan_meta:
        return PLAN_META_FIELDS
    fields: list[str] = []
    for m in plan_meta:
        for k in m:
            if k not in fields:
                fields.append(k)
    ordered = [k for k in fields if k not in ("plan_id", "reasons")]
    head = ["plan_id"] if "plan_id" in fields else []
    tail = ["reasons"] if "reasons" in fields else []
...
def write_plan_meta_csv(plan_meta: list[dict], path: str) -> None:
    """analytics ต่อ plan_id (พ่อ/แม่ หรือ ฐาน/ยอด แล้วแต่ strategy) — ไฟล์ใหม่ ไม่ยุ่ง plans.csv
    คอลัมน์ dynamic ตาม key ที่ strategy ใส่มา (ดู _plan_meta_fields)"""
    fields = _plan_meta_fields(plan_meta)
    with open(path, "w", newline="", encoding="utf-8") as f:
